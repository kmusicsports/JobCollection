...
アプリの機能仕様について記載している。3.1~3.3では、実装した機能について、3.4では、今後実装する予定の機能の一覧を示す。
...
![図: 企業の基本情報の表示機能（スクリーンショット）](docs/images/screenshots/showCompanyInfo.png)
*図: 企業の基本情報の表示機能（スクリーンショット）*

![企業の登録機能（スクリーンショット）](docs/images/screenshots/formUpdateCompanyInfoNone.png)
*図: 企業の基本情報の編集機能(データなしの状態)（スクリーンショット）*

![企業の登録機能（スクリーンショット）](docs/images/screenshots/formUpdateCompanyInfo.png)
*図: 企業の基本情報の編集機能(データありの状態)（スクリーンショット）*

![図: 企業の基本情報の更新結果（スクリーンショット）](docs/images/screenshots/resultUpdateCompanyInfo.png)
...
実装した機能および本アプリを開発するにあたっての仕様を説明する。

### 4.1. 設計資料

本アプリを設計する際に作成した資料を示す。
...
実装した機能に関するER図を下記に示す。companyテーブルは企業情報を、company_connectionテーブルは企業との接触情報を管理するためのテーブルである。
...
1. 開発チームが2人のみで、アプリもシンプルかつ小規模なものなので、他のアーキテクチャだと過剰だと考え、MVCを選択した。
2. 現代のMVCでは、Controllerが古典MVCにおけるModelの役割（ロジック）も含むことになると思われるが、単一責任の原則(SRP)の観点から、Controllerには複数の役割（ロジック）を持たせたくないと考え、ビジネスロジックの役割をServiceに、データ操作の役割をRepositoryに分割した。
...
基本的には、アーキテクチャに則ったクラス構成になっている。ServiceとRepositoryについては、オープン・クローズドの原則(OCP)の観点から、インターフェースを用意し、それを実装した(具象)クラスには依存させずに、インターフェースに依存させるようにした。
...
仕様については、基本的に2人で話し合って決めた。

- 永田: SQL、企業の基本情報管理機能
- 梶谷: 環境・基盤構築、企業管理機能、企業との接触情報管理機能
